...
button_click = pg.mixer.Sound('Assets/sfx/menu_click.mp3')
interface = pg.mixer.Sound('Assets/sfx/interface.mp3')

title_music = pg.mixer.Sound('Assets/music/Forest (rushed ver).wav')
ocean_theme = pg.mixer.Sound('Assets/music/Journey Across the Blue.wav')
forest_theme = pg.mixer.Sound('Assets/music/On The Move.mp3')
village_theme = pg.mixer.Sound("Assets/music/We're Bird People Now.mp3")
street_theme = pg.mixer.Sound('Assets/music/Long Road Ahead.mp3')
school_theme = pg.mixer.Sound('Assets/music/World Travelers.mp3')
...
def Login(email, pwd):
    ...
    submit_login = Button('rect', (size.w * 0.175, size.h * 0.645), (size.w*0.275, size.h * 0.06), 
    ...
    while True:
        ...
        for item in [to_signup, email_box, pwd_box, submit_login, quit]:
            item.Blit(0,10)
...
def Title(restart_music):
    ...
    while True:
        ...
        if enter_game:
            alpha -= 15
            if alpha < -20: In_Game_Menu(255, False)     
        else:
...
#In Game Screen
def In_Game_Menu(alpha, restart_music):
    if restart_music:
        music_channels.play(title_music, loops = -1)
    ...
    #Load Assets
    background = pg.transform.smoothscale(pg.image.load('Assets/icon/others/in_game_bg.png').convert_alpha(), (size.w*1.075, size.h*1.075))
...
def Mini_Game_Menu():
    ...
    def gameover_display_message(rbtn_image, gmo_image):
        ...
        screen.blit(gmo_image, gmo_rect)
    ...
    class Dino():
        ...
        def update(self):
            ...
            if not self.dead and self.counter % 7 == 6 and self.blinking == False:
                self.pre_score += 1
                if self.pre_score % 2 == 0:
                    self.score += 1

                if self.pre_score % 100 == 0 and self.pre_score != 0:
                    sfx_channels.play(checkPoint_sound)
    ...
    class Scoreboard():
        def __init__(self,x=-1,y=-1):
            ...
            self.image = pg.Surface((150,int(20*6/5)))
        ...
        def draw(self):
            screen.blit(coin_symbol,(size.w*0.825, size.h * 0.1))
            screen.blit(self.image, self.rect)
    ...
    def introduction_screen():
        ...
        while not starting_game:
            ...
            if pg.display.get_surface() != None:
                ...
                screen.blit(t_ground[0], t_ground_rect)
        ...
        while not g_exit:
            ...
            while not g_Over:
                ...
                if pg.display.get_surface() != None:
                    screen.fill(bg_color)
                    new_grnd.draw()
                    skyClouds.draw(screen)
                    score_boards.draw()
                    if highest_scores != 0:
                        highScore.draw()
                        screen.blit(ado_image, ado_rect)
                    cactusan.draw(screen)
                    smallBird.draw(screen)
                    gamer_Dino.draw()
...
def History_Menu():
    ...
    background = pg.transform.smoothscale(pg.image.load('Assets/icon/others/in_game_bg.png').convert_alpha(), (512, 288))
    ...
    chr_Set = Font(30).render(Updt_Lang(lang, 'History', 'Set'), True, "#FFFFFF")
    race_Len = Font(30).render(Updt_Lang(lang, 'History', 'Length'), True, "#FFFFFF")
    result = Font(30).render(Updt_Lang(lang, 'History', 'Rank'), True, "#FFFFFF")
    coins_Change = Font(30).render(Updt_Lang(lang, 'History', 'Coin'), True, "#FFFFFF")
    ...
    while True:
        ...
        screen.blit(title, title.get_rect(center = (size.w * 0.5, size.h * 0.125)))
        screen.blit(prompt, prompt.get_rect(center = (size.w * 0.5, size.h * 0.2)))
...
def View_Ranks(image_path):
    ...
    try:
        image = pg.transform.smoothscale(pg.image.load(f'{image_path}').convert_alpha(), (size.w*0.45, size.h*0.6))
        image_rect = image.get_rect(center = (size.w/2, size.h* 0.45))

    except:
        error_alpha = 255
        error = True
        
    to_text = Button ('rect', (0,0), (size.w*0.175, size.h * 0.07), None, None, None, None, '#f9b17a', '#d69869', None, None)
    to_text.rect.center = (size.w/2, size.h * 0.85)

    to_text_text = Font(int(20 * size.w/1280)).render(Updt_Lang(lang, 'History', 'Convert'), True, "#424769")

    image_error = Font(int(60*size.w/1280)).render(Updt_Lang(lang, 'History', 'Error'), True, "#FF0000")
    ...
    while True:
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.MOUSEBUTTONDOWN:
                ...
                if to_text.Click(mouse_pos) and error != True:
                    sfx_channels.play(button_click)
                    pg.image.save(screen,'Assets/temps/temp.png')
                    Loading(image_path)
        ...
        if error == False:
            ...
            to_text.Blit(0,10)
            to_text.Hover(mouse_pos, 0, 10)
            screen.blit(to_text_text, to_text_text.get_rect(center = to_text.rect.center))
...
def Choose_Character_Set(alpha, chr_set, race_len):
    ...
    bg_List = ['Assets/background/ocean.png', 
                ...
                'Assets/background/school/school_race.jpg',
                'Assets/icon/others/in_game_bg.png']
    ...
    setE = Button('image', None, None, 'Assets/background/school/school_race.jpg', (size.w * 0.15, size.h * 0.2), None, None, None, None, 'Assets/background/school/school_race.jpg', (size.w * 0.9, size.h * 0.35))
            ...
            if event.type == pg.MOUSEBUTTONDOWN:
                ...
                if change_menu == False:
                    ...
                    if enter.Click(mouse_pos) and race_len != 3 :
                        sfx_channels.play(interface)
                        current_user.Update_Coin(-100)
                        current_user.Save_History(preload_chr_set[chr_set], preload_race_len[race_len], "5th", -200, None)
...
# Thiết lập kích thước màn hình
    ...
    match theme:
        case 0:
            music_channels.play(ocean_theme, loops = -1)
        case 1:
            music_channels.play(forest_theme, loops = - 1)
        case 2:
            music_channels.play(village_theme, loops = - 1)
        case 3:
            music_channels.play(street_theme, loops = - 1)
        case 4:
            music_channels.play(school_theme, loops = - 1)
    ...
    name_list = ['Ng.Thien Nhan', 'Quoc Kien', 'Van Phuc', 'Vo Thien Nhan', 'Nhat Khang']
    ...
    class Char:
        def __init__(self, x, y, speed, name, image_path):
            ...
            self.orientation = 1
            self.baseY = self.y
            self.laps_display = Draw_to_Screen('text', None, None, None, None, f'{self.laps}/{length}', Font(int(30 * size.w / 1280)), '#FFFFFF', (self.x - 20 * size.w / 1280, self.y + 20 * size.h / 720))
            if theme == 3:
                self.name_display = Draw_to_Screen('text', None, None, None, None, f'{self.name}', Font(int(30 * size.w / 1280)), '#FFFFFF', (self.x + 40 * size.w / 1280, self.y + 0.15 * size.h))
            else:
                self.name_display = Draw_to_Screen('text', None, None, None, None, f'{self.name}', Font(int(30 * size.w / 1280)), '#FFFFFF', (self.x + 40 * size.w / 1280, self.y + 0.1 * size.h))
            self.rank_display = Draw_to_Screen('text', None, None, None, None, "", Font(int(30 * size.w / 1280)), '#FFFFFF', (0,0))
            self.player_chose = Draw_to_Screen('text', None, None, None, None, "", Font(int(30 * size.w / 1280)), '#FFFFFF', (0,0))
    ...
    def show_menu():
        ...
        charImage = Draw_to_Screen('text', None, None, None, None, '', 
                                        Font(int(40 * size.w / 1280)), '#FFFFFF', (size.w * 0.60, size.h * 0.5))
        rename_box = Button('rect', (size.w * 0.35, size.h * 0.7), (size.w*0.275, size.h * 0.1), None, None, None, None, '#FFFFFF', '#FFFFFF', None, None)
        if theme == 4:
            charname_text = Draw_to_Screen('text', None, None, None, None, Updt_Lang(lang, 'Core_Game', 'Fixed_Name'), Font(int(40 * size.w/1280)), '#FFFFFF', (size.w * 0.5, size.h * 0.65)) 
        else:
            charname_text = Draw_to_Screen('text', None, None, None, None, Updt_Lang(lang, 'Core_Game', 'Choose_Name'), Font(int(40 * size.w/1280)), '#FFFFFF', (size.w * 0.5, size.h * 0.65)) 
      
        while running:
            mouse_pos = pg.mouse.get_pos()
            screen.blit(bg,(0,0))
            text = Draw_to_Screen('text', None, None, None, None, Updt_Lang(lang, 'Core_Game', 'Title'), 
                        Font(int(70 * size.w / 1280)), '#FFFFFF', (size.w * 0.5, size.h * 0.3))
            
            currentChar = Draw_to_Screen('text', None, None, None, None, Updt_Lang(lang, 'Core_Game', 'Selected'), 
                        Font(int(50 * size.w / 1280)), '#FFFFFF', (size.w * 0.4, size.h * 0.5))
            
            Start = Button('rect', (size.w*0.4, size.h * 0.85), (size.w*0.175, size.h * 0.075), None, None, None, None, '#FFFFFF', '#D3D3D3' , None, None)
            Start_text = Draw_to_Screen('text', None, None, None, None, Updt_Lang(lang, 'Core_Game', 'Start'), Font(int(40 * size.w / 1280)), '#000000', Start.rect.center)
            
            rename_text = Draw_to_Screen('text', None, None, None, None, player_name, Font(int(40 * size.w / 1280)), '#000000', rename_box.rect.center)
            
            text.Blit(0,0)
            currentChar.Blit(0,0)
            charImage.Blit(0,0)
            Start.Blit(0,10)
            Start.Hover(mouse_pos, 0, 10)
            Start_text.Blit(0,0)
            rename_box.Blit(0,10)
    ...
    Finish = Button('rect', (size.w*0.4, size.h * 0.65), (size.w*0.175, size.h * 0.075), None, None, None, None, '#FFFFFF', '#D3D3D3' , None, None)
    Finish_text = Draw_to_Screen('text', None, None, None, None, Updt_Lang(lang, 'Core_Game', 'Next'), Font(int(40 * size.w / 1280)), '#000000', Finish.rect.center)
    ...
    while running:
        # Xử lý các sự kiện
                ...
                if (event.type == pg.MOUSEBUTTONDOWN) and all_Finish:
                        ...
                        if Finish.Click(pos):
                            ...
                            match ranking_list.index(player_choice + 1):
                                case 0:
                                    current_user.Update_History('5th', -100)
                                case 1:
                                    current_user.Update_History('4th', -100)
                                case 2:
                                    current_user.Update_History('3rd', -100)
                                case 3:
                                    current_user.Update_History('2nd', -100)
                                case 4:
            ...
            for char in chars:
                char.act_i = char.draw(char.act_i, char.status)
                char.move()
                char.laps_display = Draw_to_Screen('text', None, None, None, None, f'{char.laps}/{length+1}', Font(int(20 * size.w / 1280)), '#FFFFFF', (char.x - 15 * size.w / 1280, char.y + 20 * size.h / 720))
                if theme == 3:
                    char.name_display = Draw_to_Screen('text', None, None, None, None, f'{char.name}', Font(int(30 * size.w / 1280)), '#FFFFFF', (char.x + 40 * size.w / 1280, char.y + 0.15 * size.h))
                else:
                    char.name_display = Draw_to_Screen('text', None, None, None, None, f'{char.name}', Font(int(30 * size.w / 1280)), '#FFFFFF', (char.x + 40 * size.w / 1280, char.y + 0.1 * size.h))
            ...
            for i, char in enumerate(chars):
                if (char.x >= 0.92*size.w and char.laps % 2 == 0) or (char.x <= 0.05*size.w and char.laps % 2 == 1):
                    if char.laps == length and i not in finish_order:
                        ...
                        char.laps += 1
                        char.finished = True
                        char.laps_display = Draw_to_Screen('text', None, None, None, None, f'{char.laps}/{length+1}', Font(int(40 * size.w / 1280)), '#FFFFFF', (char.x - 30 * size.w / 1280, char.y))
...
def Show_Result(ranking_list, player_choice, game_theme, size, chars_list):
    ...
    class Stage:
        ...
        def display(self, screen, x, y):
            ...
            text = Font(int(30 * size.w/1280)).render(self.name, True, (0, 0, 0))
    ...
    spacing, running, animation_speed, delay_between_stages = 8, True, 5, 60
    ...
    Next = Button('rect', (size.w*0.4, size.h * 0.9), (size.w*0.175, size.h * 0.075), None, None, None, None, '#FFFFFF', '#D3D3D3' , None, None)
    Next_text = Draw_to_Screen('text', None, None, None, None, Updt_Lang(lang, 'Core_Game', 'Next'), Font(int(40 * size.w / 1280)), '#000000', Next.rect.center)
    ...
    while running:
        ...
        text_surface = Font(int(30 * size.w/1280)).render(f'{Updt_Lang(lang, 'Show_Result', 'Rank')} {rank}', True, (255, 255, 255))
    ...
    Result_text = Draw_to_Screen('text', None, None, None, None, 'RESULT', Font(int(60 * size.w / 1280)), '#FFFFFF', (size.w * 0.1, size.h * 0.05))
    for i in range(5):
        j = ranking_list[i] - 1
        char = chars_list[j]
        rank = 5 - i - 1
        char.rank = rank
        char.rank_display = Draw_to_Screen('text', None, None, None, None, f'{char.name} : {rank_list[rank]}', Font(int(25 * size.w / 1280)), '#FFFFFF', (0,0))
        char.rank_display.rect.center = (size.w * 0.1325, size.h * (0.175 + 0.139 * rank))
        if j == player_choice:
            char.player_chose = Draw_to_Screen('text', None, None, None, None, "Chosen by player", Font(int(25 * size.w / 1280)), '#e8bd3d', (size.w * 0.425, size.h * (0.175 + 0.14 * rank)))
        else:
            char.player_chose = Draw_to_Screen('text', None, None, None, None, "", Font(int(25 * size.w / 1280)), '#FFFFFF', (size.w * 0.4, size.h * (0.175 + 0.14 * rank)))
    horizontal_lines = [Draw_to_Screen('rect', (size.w * 0, size.h * (0.1 + 0.139 * i)), (size.w * 0.625, size.h * 0.005), None, None, None, None, '#ffffff', None) for i in range(7)]
    vertical_lines = []
    vertical_lines.append(Draw_to_Screen('rect', (size.w * 0, size.h * 0.1), (size.w * 0.005, size.h * 0.75), None, None, None, None, '#ffffff', None))
    vertical_lines.append(Draw_to_Screen('rect', (size.w * 0.25, size.h * 0.1), (size.w * 0.005, size.h * 0.75), None, None, None, None, '#ffffff', None))
    vertical_lines.append(Draw_to_Screen('rect', (size.w * 0.6025, size.h * 0.1), (size.w * 0.005, size.h * 0.75), None, None, None, None, '#ffffff', None))
    test = pg.surface.Surface((size.w * 0.6075, size.h * 0.8))
    test.set_alpha(200)
    test.fill(0)
    Next.rect.bottom += size.h * 0.005
    running = True
    while running:
        mouse_pos = pg.mouse.get_pos()
        screen.fill(0)
        ...
        screen.blit(test, (size.w * 0.175, size.h * 0.06))
...
def Loading(image_path):
    counter = 0
    call = randrange(-350, -100)
    bg = pg.image.load('Assets/temps/temp.png').convert_alpha()
    ...
    while True:
        ...
        screen.fill(0)
        loader = pg.transform.rotate(pg.image.load('Assets/icon/others/loading.png').convert_alpha(), counter)
        bg.set_alpha(150)

        screen.blit(bg, (0,0))
        screen.blit(loader, loader.get_rect(center = (size.w/2, size.h/2)))

        if counter < call:
            if Convert(image_path):
                return
